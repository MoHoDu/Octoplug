...
- `Assets/03_Prefabs/Multitaps/Multitap.prefab` — the production variable-capacity PowerStrip source. The five size-specific Multitap prefabs remain compatibility/reference assets.
- `Assets/03_Prefabs/Wall_Outlets/Wall_Outlet.prefab` — the production variable-capacity Wall Outlet source. `Wall_Outlet_One.prefab` through `Wall_Outlet_Five.prefab` remain unchanged compatibility/reference assets, not production scene dependencies.
...
- Production Room content reachability explicitly requests the terminal's front/room-facing Approach Point. This is required on a shared Wall: the Socket's authored sub-cell offset can make the adjacent Room's cell geometrically closer even though the Outlet belongs to the promoted Room. The general cable-routing call keeps its existing default; pointer acquisition enforces the approach side separately.
...
- `PowerValidationService.IsSocketSourceLive` rejects inactive Sockets before evaluating their owner. An active Wall Outlet Socket has no `PowerStrip` ancestor, so only the House budget applies through it (matches "Wall Outlet has no own limit"). A PowerStrip's Socket additionally requires that strip's own `AllowedPowerWatts` to pass (`SocketConnector.GetComponentInParent<PowerStrip>()`).
...
- **Finalized allowance balance (2026-09-19):** House current/initial allowance is `8` with independent maximum `22`. PowerStrip current/initial allowances are independent per prefab (`One=3`, `Two=3`, `Three=4`, `Four=5`, `Five=5`), with independent maximum `10` per strip. Socket count never derives Allowed Power and Allowed Power never derives socket count.
- Runtime strengthening is exposed only through exact production operations: `PowerStrip.TryUpgradeActiveSocketCount`, `PowerStrip.TryUpgradeAllowedPowerWatts`, and `CableInfo.TryUpgradeCableLength`. Each increments its own authority by exactly one and leaves the other two values untouched. Socket expansion keeps using the owner-aware transactional footprint reservation; Allowed Power is capped at `min(MaxAllowedPowerWatts, 10)`; Cable Length has no added balance cap.
- `CableInfo.cableLength` is the sole authoritative Cable Length state. `CableLengthChanged` is published once after a successful change; routing invalidates its loose-route cache and immediately reruns the normal length-aware preview only while a Plug drag is active. An idle or connected Plug is not moved or disconnected by the upgrade.
- These runtime mutations last for the Play session only. Reward selection/generation, persistence, and production upgrade UI remain outside this contract.
- UI capacity follows the same independent contract: House has 22 authored icons; the shared PowerStrip meter has 10 authored and serialized icons, one per supported watt. Icon count must never clamp or mutate authoritative Allowed Power.
...
- `TryUpgradeActiveSocketCount` is the public atomic, increase-only in-place transaction; the arbitrary requested-count helper remains private. It rejects out-of-range or incomplete configuration, keeps every decrease unsupported, and preflights the prospective placement reservation before committing. Failure leaves count, visuals, reservation, connections, Allowed Power, Powered state, Flow, PowerInfo, and binding unchanged.
...
### Persistent Wall Outlet socket capacity (established, TASK-20260918-007 continuation)

- `Assets/03_Prefabs/Wall_Outlets/Wall_Outlet.prefab` is the production source. Its persistent `Socket01`–`Socket05` and `Whole01`–`Whole05` identities are authored once and shared through `SocketModuleLayout`; runtime count changes never instantiate, destroy, replace, rename, or reconnect them.
- `WallOutlet.InitialSocketCount` initializes runtime `ActiveSocketCount` in the supported range 1–5. `TryUpgradeActiveSocketCount` grows exactly one contiguous module, keeps Whole01 First active, activates only the final active Whole's End, publishes its count event once, and notifies graph topology only after success. Count decrease remains unsupported.
- Geometry is the union of authored Whole01 First, Body01–N, and EndN `BoxCollider2D` components. Inactive Sockets are excluded from interaction, magnetic selection, direct connection, validation, source-live checks, usage, and graph traversal.
- Wall Outlet remains a fixed House-backed terminal root: no Cable/Plug, drag, placement reservation, local allowance, powered cascade, or PowerInfo. Count changes never mutate House allowance. Active Products contribute through House accounting only.
- Terminal routing remains `Grid path → short room-side approach cell → exact Socket endpoint`. Opposite-side acquisition is rejected, a Socket is never a portal, and cross-room traversal remains Door-only.
- `InfiniteMode.unity` now contains exactly one unified instance under `Wall_Outlets`, source GUID `c7bc3b7b69ecb644cbe01fb717727a5c`, local position `(4, 1, 0)`, rotation Z `90°`, scale `(1, 1, 1)`, and initial count `1`. The staged scene contains no legacy Wall Outlet GUID. House allowance/max remain `8 / 22`; all five legacy prefab assets are unchanged.
- Legacy `Wall_Outlet_One`–`Wall_Outlet_Five` assets remain for compatibility/reference only. A future Room Generator must instantiate the unified source, orient it from its mounting Wall, and supply count 1–5 through a contract shaped as `CreateWallOutlet(wall, position, socketCount)`; that API is documented direction, not implemented production integration.

## Current Verification Status (TASK-20260918-007)

- Official EditMode verification: **PASS 11/11** — PowerStrip **4/4**, WallOutlet **7/7**.
- Build: **PASS** with 0 warnings/0 errors. Unity compile: **PASS / up_to_date**. Runtime verification: **PASS**.
- Console delta: **RELIABLE**; new errors **0**, new warnings **0**. `verify-fast.ps1`: **PASS** (context-budget warnings remain).
- Real pointer/visual behavior remains **HUMAN_VERIFY_REQUIRED**. Official EditMode tests do not replace the seven focused Play Mode checks.

## Planned, Not Established

- Head-drag-beyond-Cable-Length behavior remains a pending Human Decision; runtime Cable Length strengthening does not alter Head drag.
- Production Room Generation, Door procedural generation, product outline feedback, audio, Reward mechanics, persistence, and production upgrade UI remain unimplemented.
