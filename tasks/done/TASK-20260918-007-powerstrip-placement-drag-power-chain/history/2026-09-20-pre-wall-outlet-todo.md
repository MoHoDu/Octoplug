...
## R. Runtime upgrade controls (2026-09-20, current)

- [x] Authorize only Active Socket Count +1, Allowed Power +1, Cable Length +1, and a removable Play Mode custom Inspector; keep Reward System, persistence, production gameplay UI, scene migration, Wall Outlet work, and Head-drag length enforcement out of scope.
- [x] Added exact public production +1 APIs and typed failures; removed broad arbitrary-value mutation from the public caller surface.
- [x] Preserved the verified owner-aware transactional footprint expansion and persistent authored identities/connections.
- [x] Added `CableInfo.CableLengthChanged`, invalidated loose-route cache, and recomputed only an active Plug drag through the existing length-aware preview path.
- [x] Refresh the visible Product cable label and PowerStrip PowerInfo through authority events.
- [x] Read back the unified `PowerInfo.prefab` through the matching isolated Editor: ten authored `Image`s, ten serialized binding entries, zero nulls; no repair or prefab save was needed.
- [x] Added `Assets/Editor/Power/PowerStripEditor.cs` with runtime values and three Play Mode-only buttons that call production APIs only.
- [x] Added focused ephemeral matching-Editor verification instead of broad test-assembly restructuring; Unity Test Framework still discovers zero project tests (`NO_PROJECT_TESTS`).
- [x] Replaced the earlier private-field baseline setup with a source-prefab verifier that reaches `2 / 5 / 3` only through production +1 APIs, then verifies `3 / 5 / 3 → 3 / 6 / 3 → 3 / 6 / 4`, identity/state independence, blocked-expansion rollback/old claim preservation, and allowance cap 10 (`PASS|checks=91`). The separate real active-drag Cable reach/cache verifier remains `PASS|checks=9`. Prior connected 2→3 preservation remains a verified regression constraint; real Inspector interaction is still Human Verification. Synthetic invalid/non-finite-state rejection was checked separately with explicit malformed-state injection and is not presented as a production-path upgrade.
- [x] C# build PASS (0 warnings/errors), Unity recompile clean/up-to-date, `verify-fast.ps1` PASS (warnings only), `git diff --check` clean apart from line-ending notices, exact-worktree `verify-unity.ps1` PASS, prefab readback PASS, and no new Console errors after verification cursor. Unity EditMode=0 and PlayMode=0: **NO_PROJECT_TESTS**.
- [ ] Run exactly three real Inspector-button Human Verification scenarios and keep status `HUMAN_VERIFY_REQUIRED` until they pass.

## S. Minimal reusable Unity verification foundation (2026-09-20, current)

- [x] Authorized only a reusable Editor fixture, one minimal EditMode test assembly, four Multitap regressions, one `verify-task.ps1` entry point, Console delta tracking, and concise MCP verification documentation.
- [x] Add `UnityVerificationFixture` with isolated Grid context, prefab instantiation, production-API state upgrades, production connection helpers, owner-aware reservation support, Wall Outlet factory support, and deterministic cleanup.
- [ ] Add and discover exactly four official EditMode tests: upgrade independence, connected identity preservation, blocked expansion rollback, and inactive Socket rejection. **Implemented and discovered as 4; the first official run failed 0/4 before assertions because the EditMode Grid service lifecycle was not initialized. The fixture now invokes the real `Awake` lifecycle without private-state writes, but the corrected run is blocked by the stale Pipeline execution gate.**
- [x] Add `scripts/verify-task.ps1` with exact-project-path build/compile/discovery/run/Console-delta/verify-fast orchestration and stable summary labels.
- [x] Document the single reusable command in `harness/mcp/README.md`.
- [ ] Run the foundation once end-to-end, record discovered/passed counts and Console delta, and keep the three real Inspector-button scenarios pending. **Current bounded wrapper result: Build PASS, verify-fast PASS, Unity Compile/Tests FAIL because every Pipeline command times out, Console delta UNRELIABLE, Overall FAIL.**

## T. Unified variable-count Wall Outlet continuation (2026-09-20, current)

- [x] Kept the work in TASK-007 and authorized the constrained unified Wall Outlet continuation; no new Task created.
- [x] Declared `Wall_Outlet.prefab`, shared/runtime scripts, official tests, verifier/docs, and the later one-for-one `InfiniteMode.unity` migration as Exclusive Assets. Legacy Wall Outlet prefab assets are read-only inputs.
- [ ] Extract the narrow persistent socket-module layout while preserving Multitap serialized behavior and PowerInfo positions.
- [ ] Add Wall Outlet initial/runtime count, increase-only production API, typed failure, event, and topology notification.
- [ ] Make Socket activity/source-live/graph gates WallOutlet-aware with no prefab-name branching.
- [ ] Compile runtime code before serialized asset mutation.
- [ ] Wire and read back the unified `Wall_Outlet.prefab` through the exact matching clean Editor; preserve visual design and terminal behavior.
- [ ] Add official Wall Outlet EditMode tests and keep all four Multitap regressions green.
- [ ] Broaden `verify-task.ps1` to the combined suites and explicit `Console Delta` reporting.
- [ ] Run build/compile/tests/Console-delta/verify-fast before scene migration; report every failure or environment limitation honestly.
- [ ] Migrate each legacy `InfiniteMode.unity` Wall Outlet instance one-for-one through the exact matching Editor, preserving state and unrelated changes; save only the scene and read it back.
- [ ] Update domain/MCP/handoff records and request exactly the seven focused Human Verification checks.

## Next actions (current)

1. Implement the shared layout and Wall Outlet runtime authority, then compile.
2. Use the matching Editor for unified-prefab wiring, add/run the combined official tests, and require automated correctness before scene migration.
3. Preserve `.vscode/settings.json`, root `TODO.md`, every unrelated scene change, and all legacy Wall Outlet assets; never reset/discard or hand-edit Unity YAML.
4. Keep `active` / `HUMAN_VERIFY_REQUIRED`; do not commit, push, mark DONE, implement Reward System/production Room Generation, or claim completion before Human Verification PASS.
